 ...
 * 函 数 返 回：无
 * 作       者：适配标准库
 * 备       注：需根据MCU型号调整DMA通道和时钟
**********************************************************/
void bsp_spi_dma_tx_init(void) {
    NVIC_InitTypeDef NVIC_InitStructure;

    // 1. 使能DMA时钟（示例：STM32F4 DMA2时钟，根据MCU调整）
    ...
    // 3. 配置DMA核心参数（全局结构体，后续仅动态修改内存地址和长度）
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;  // 按需调整
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1;         // 按需调整
...
//    }
    ...
    while (remain_len > 0) {
        ...
        write_len = (write_len > remain_len) ? remain_len : write_len;
        write_len = (write_len > DMA_TX_BUFFER_SIZE) ? DMA_TX_BUFFER_SIZE : write_len;

        // 打印当前批次关键信息（方便定位漏写批次）
        printf("当前批次：地址=0x%X，长度=%d字节，剩余=%d字节\n", curr_addr, write_len, remain_len);
