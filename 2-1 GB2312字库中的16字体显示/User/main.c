...
{
    uint8_t read_buff[32];  // 读取缓冲区（24X24字是72字节=1个汉字）


    // 校验字库地址对于的字模
    W25Q128_read(read_buff, W25Q128_GBK_ADDR +  (1000)*32 , 32);
...
//	Verify_GBK_Font();
	...
	//从Flash中读取的
	EPD_ShowChineseString_flash(0,60,"楔些歇蝎鞋斜胁谐写芤",FONT_16X16,BLACK);
	EPD_Display(ImageBW);
	EPD_Update();
	EPD_DeepSleep();
